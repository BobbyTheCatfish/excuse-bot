...
async function register() {
  ...
  const commandPath = path.resolve(require.main ? path.dirname(require.main.filename) : process.cwd(), "./");
  ...
  const files = { commands: Object.fromEntries((global?.data ?? []).map(cmd => {
    return [
      `${getCommandType(cmd.type)}${cmd.name[0].toUpperCase()}${cmd.name.substring(1).toLowerCase()}`,
      cmd.id
    ];
  }).sort((a, b) => a[0].localeCompare(b[0]))) };
  fs.writeFileSync(path.resolve(__dirname, "./snowflakes-commands.json"), JSON.stringify(files, null, 2));
  const oldExample = require("./snowflakes-commands-example.json");
  ...
  if (diff.length > 0) fs.writeFileSync(path.resolve(__dirname, "./snowflakes-commands-example.json"), JSON.stringify({ commands: Object.fromEntries(newKeys.map(f => [f, ""])) }, null, 2));
